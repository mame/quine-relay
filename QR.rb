...
m"}`n.m                       ethod~public~static~ma!([L#{S="java/lang/"}SJ;)V~;               ]`n.limit~stack~
2`nget                      static~#{S}System/out~L#$T;`nldc~"#{e[%(class~QR{publ               ic~static~void~
ma!(S                      J[]v){SJ~c[]=new~SJ[8000],y="",z=y,s="#{z=t=(0..r=q=126)              .map{|n|[n,[]]
};a=                     [];%(@s=global[#{i=( s=%(PR~"#{Q["Z"+E[%(all:`n`t@Zf~%s~"#{             e[%(.assembly~
t{}.                    method~static~void~M   X{.entrypo!t~ldstr"#{e["require('util')            .Z#{E["#impor
t"+                    M["Z_sJ"+E["Zf"+E[%(  s  ay"#{e["#$D(output);beg!~H(#{((p="eval"            ;%($_="#{s=%
(<?                   php~function~f($n){r  etu  rn~str_repeat("Y",$n);};$f="f";echo#{V            [Q[E["!t~mX{
H#{       E[         "qr:-H('#{Q[e[%(for~  l~!#{  E[e[d[%(eval$s=%q(#$s))]]]}.split("Yn"            ):Z('cat("s
ay~                 Y"'+l+'Y"Yn")'))],?'  ]}')."]  };return~0;}"]],"{$f(",")}"]}?>);(s+N*           (-s.size%6)
).un               pack("B*                                     ")[0].K(/.{6}/){n=$&.to_i           ~2;((n+14)/
26*6+        n+4  7).chr}}";  s|.|$n=  ord$&;substr~  unpack(  B8,chr$n-!t($n/32)*6-41),2|          eg;Z~pack"B
*",$_)       .scan(/[~,-:A-z]  +|(.)  /){p="s++#{$1?"  chr~#  {$1.ord}+e":$&+?+};"+p};p)).          K(/.{1,255}
/){|s|"'#   {s}',"}}'')end."]}  "`n  end)+N]]]]}"]}"ca  ll~  void~[mscorlib]System.Console          ::Write(sJ)
ret})]}")],/[~^ `t;"(){}`[`]]/]  }  ~B`x59E)).size+1}x~  i  8]c"#{s.K(/[^"]/){"^%`x58"%$&.          ord}}^00"de
clare~i32@puts( i8*)def!e~i32@mX   {%1=call~i32@puts(i8*   getelementptr([#{i}x~i8]*@s,i32          ~0,i32~0))r
et~i32~0}).byte s{|n|r,z=z[n]||  (  a<<r;q<5624&&z[n]=[  q  +=1,[]];t[n])};a<<r;t=[*     4          3..123]-[64
,*92..96];a.map  {|n|t[n/75].c  hr+  t[n%75].chr}*""}"  ;!t  ~i=0,n=0,q=0               ,          t  ;for(;++n
<126;)c[n]=""+(c har)n;for(;i  <s.le  ngth();){t=s.ch  arAt(  i);q=q*7                  5          +   t-t/64-t
/92*5-43;if(i++%  2>0){y=q<n  ?c[q]:y  ;c[n++]=z+y.c  harAt(0  );System.out.Z          (          z     =c[q]);
q=0;}}}})]}"`n!vo kevirtual                                     ~#$T/Zln(L#{           S          }      SJ;)V`
nreturn`n.end~met  hod)+N]};H("DO,1<-#"|  |*s);s?  while~t:=ord(move(1))do{i +:=1;u:= -          i       ;every
