...
{s.gsub(t){t+t}};Q=->(s,t=?$){s.gsub(t){B+$&}};puts(eval(%q(%(objectXQRXextendsXApp{H#{E[%((displayX"#{e[%(HfX%
sX"#{Q[e["TranscriptXshow:X'#{d[%(putsX[r egsubX-  allX{.}X  "#{Q[e[%[intXK(){sJXs=#{E[%(withXAda.Text_Io;proce
dureXqrXisXbeginXAda.Text_Io.Put _Line   ("#{     d[%(      BEGINXH("#{d[%(BEGIN{s=#{E[%(forXbXinXSystem.Text.A
SCIIEncoding().GetBytes(#{Q[E[  "#i     n                            clude<stdio.h>`nintXK(){puts#{E["#include<
iostream>`nintXK(){std ::cou    t                                   <<#{E[%(classXProgram{publicXstaticXvoidXMa
in(){System.Console.W  rit                                                    e(#{E[%((doseq[s(lazy-cat["IDENTI
FICATIONXDIVISION.""    P                                                    ROGRAM-ID.XQR.""PROCEDUREXDIVISION
."'DISPLAY](mapX#(s                                                                 tr"XXXX^""(.replace%1"^"""^
"^"")"^"&")(re-seq                                                                 X#".{1,45}""#{e["console.log
"+E[%((write-line                                                                       "#{Q[%(X:XAX."XXXXXXXXX
"X;X:XBXAX."XWRI                                                                       TE(*,*)'"XAX;X:XCXBXTYPE
X."X'"XCRX;X:X                                    DXS"Xprog                                ramXQR"XCXS^"XHX^"(&
"XCXS^"X#{e[                              %(packageXK;import"fmt";f                       uncXK(){fmt.Print#{E[
%(H'#{e[("K                           =putStr"+E[%(procedureXK();i:=c:=0                   ;s:=#{E[%(.classXpub
licXQR`n.                         superXjava/lang/Object`n.methodXpublicXst                  aticXK([Ljava/lang
/SJ;)V`n                        .limitXstackX2`ngetstaticXjava/lang/System/out                XLjava/io/PrintSt
